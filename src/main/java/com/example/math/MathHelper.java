...
public class MathHelper {
	...
	public static List<Double> getFixLables(final double start,
			final double end, final int count) {
		List<Double> labels = new ArrayList<Double>();
		double per = Math.abs(start - end) / count;

		for (int i = 0; i < count; i++) {
			labels.add(start + i * per);
		}
	...
	public static List<Double> getYLabels(final double start, final double end,
			final int approxNumLabels, final boolean iszero) {
		List<Double> labels = new ArrayList<Double>();
		if (approxNumLabels <= 0) {
			return labels;
		}
		FORMAT.setMaximumFractionDigits(5);
		double[] labelParams = computeYLabels(start, end, approxNumLabels,
				iszero);
		// when the start > end the inc will be negative so it will still work
		int numLabels = 1 + (int) (((float) labelParams[1] - (float) labelParams[0]) / labelParams[2]);
		// we want the range to be inclusive but we don't want to blow up when
		// looping for the case where the min and max are the same. So we loop
		// on
		// numLabels not on the values.
	...
	private static double[] computeYLabels(final double start,
			final double end, final int approxNumLabels, final boolean iszero) {
//		if (Math.abs(start - end) < 0.0000001f) {
//			return new double[] { start, start, 0 };
//		}
		double s = start;
	...
	/**
	 * Given a number, round up to the nearest power of ten times 1 or 5.
	 * 
	 * @param val
	 *            the number, it must be strictly positive
	 */
	public static double roundUp1or5(final double val) {
		int exponent = (int) Math.floor(Math.log10(val));
		double rval = val * Math.pow(10, -exponent);
		if (rval > 5.0) {
			rval = 10.0;
